...
parser.add_argument('--router_mac', type=str, dest='router_mac', help='Specify the MAC of your router', required=True)

args = parser.parse_args()

#iface="wlp2s0"
iface = args.iface
#router_mac="AC:F8:CC:05:EE:2E" 
                                ...
                                if db_list:
                                        if 0 < len(db_list) < 10:
                                                ...
                                                if average - 20 < dbm_signal < average + 20:
                                                        ...
                                                        log = "Probe response detected on MAC address: " + str(pkt.addr2).upper() + ", of the value: " + str(dbm_signal) + " dBm, to the router: " + str(pkt.info)
                                                        logging.warning(log)
                                                        msg = "Fake AP found."
                                                        log_to_file(str(pkt.addr1),str(pkt.addr2), msg, "ALERT")
                                else:
                                        db_list.append(dbm_signal)
                        log = " [ " + str(time) + " ] Probe response detected on MAC address: " + str(pkt.addr2).upper() + ", of the value: " + str(dbm_signal) + " dBm, to the router: " + str(pkt.info)
                        msg = "Probe Response recieved"
                        log_to_file(str(pkt.addr1),str(pkt.addr2), msg, "INFO")
                ...
                if pkt[scapy.ARP].op == 2:
                        try:
                                ...
                                if real_mac != response_mac:
                                        log=f"Man in The Middle detecred on MAC: {real_mac.upper()}, MAC of the attacker: {response_mac.upper()} !!"
                                        logging.warning(log)
                                        msg = "MiTM attack"
...
def log_to_file(source_mac, dest_mac, alert, type):
        '''A function that saves logs to a file, which can be integrated with a data monitoring and visualization system (e.g. Splunk)'''
        message = "type= " + type +", source_mac= " + source_mac + ", dest_mac= " + dest_mac + ", alert= " + alert + "\n"
        with open("logfile.log", "a") as logfile:
                logfile.write(message)

print(r"""
         ___.          .__    .___      
         \_ |____  _  _|__| __| _/______
          | __ \ \/ \/ /  |/ __ |/  ___/
          | \_\ \     /|  / /_/ |\___ \ 
          |___  /\/\_/ |__\____ /____  >
              \/               \/    \/  
